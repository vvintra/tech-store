...
export default function XiaomiPage() {
  ...
  const allXiaomi = getProductsByCategory("xiaomi")
  ...
  const filteredProducts = allXiaomi.filter((phone) => {
    if (filterBy === "all") return true
    if (filterBy === "new") return phone.condition === "Nuevo"
    if (filterBy === "used") return phone.condition === "Usado"
    if (filterBy === "sale") return phone.badge === "Sale"
    return true
  })
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div>
                <h1 className="text-3xl font-bold text-gray-900">Xiaomi | Redmi</h1>
                <p className="text-gray-600 mt-1">
                  Descubre la mejor selección de Xiaomi y Redmi ({allXiaomi.length} productos disponibles)
                </p>
              </div>
            </div>
          </div>
                  ...
                  <div className={`${viewMode === "list" ? "flex justify-between items-start" : ""}`}>
                    <div className={`${viewMode === "list" ? "flex-1" : ""}`}>
                      ...
                      <div className="flex items-center text-sm text-gray-500 mb-3">
                        ...
                        {phone.storage && phone.color && <span className="mx-2">•</span>}
                        ...
                        <div className="flex items-center">
                          {[...Array(5)].map((_, i) => (
                            <Star
                              key={i}
                              className={`h-4 w-4 ${
                                i < Math.floor(phone.rating) ? "text-yellow-400 fill-current" : "text-gray-300"
                              }`}
                            />
                          ))}
                        </div>
                        <span className="text-sm text-gray-500 ml-2">({phone.reviews})</span>
                      </div>

                      <div className="space-y-2">
                        <div className="flex items-baseline space-x-2">
                          <span className="text-2xl font-bold text-gray-900">${phone.price.toLocaleString()}</span>
                          {phone.originalPrice && (
                        ...
                        <p className="text-xs text-gray-500">Stock: {phone.stock} unidades</p>
                      </div>
                    </div>
                    ...
                    <div className={`${viewMode === "list" ? "ml-6" : "mt-4"}`}>
                      ...
                      >
                        {phone.stock === 0 ? "Sin Stock" : "Agregar al Carrito"}
                      </Button>
                    </div>
                  </div>
                </CardContent>
